...
namespace audio {
  ...
  void Synthesizer::prepare(const StreamSpec& spec)
  {
    ...
    noise_envelope_.reserve(20);
    tone_envelope_.reserve(20);
  ...
  void Synthesizer::update(ByteBuffer& buffer, const SoundParameters& params)
  {
    ...
    tone_envelope_.clear();
    noise_envelope_.clear();
    ...
    filter::get<filter::std::Gain>    (noise_pipe_).setEnvelope (&noise_envelope_);
    ...
    filter::get<5> /* Gain */ (tone_pipe_).setEnvelope (&tone_envelope_);
  ...
  // static
  void Synthesizer::buildEnvelope(filter::Automation& envelope,
                                  ...
                                  float decay, EnvelopeRampShape decay_shape)
  {
    ...
    // Note: If the maximum number of filter::Automation points generated by
    //       this function changes, the reserve() calls in Synthesizer::prepare
    //       must be adapted too to prevent allocations during rt processing.

    envelope.append({{0ms, 0.0f}});
