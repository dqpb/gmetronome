...
namespace {
                                                       ...
                                                       const ActionDescription& action_description)
  {
    Glib::RefPtr<Gio::SimpleAction> action = Gio::SimpleAction::create(
      action_name,
      action_description.parameter_type,
      action_description.initial_state );
    
    if (action_description.state_hint)
      action->set_state_hint(action_description.state_hint);
    
    action->set_enabled(action_description.enabled);
                     ...
                     const ActionHandlerMap& handler)
{
  for (const auto& [action_name, action_handler] : handler)
  {
    ...
    auto action_description_it = descriptions.find(action_name);
    if (action_description_it != descriptions.end())
    {
      auto action_description = action_description_it->second;
      ...
      if (std::holds_alternative<ActionHandlerSlot>(action_handler))
      {
        ...
        Glib::RefPtr<Gio::SimpleAction> simple_action =
          create_custom_action(action_name, action_description);
